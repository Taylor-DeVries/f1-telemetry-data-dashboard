...
export default function Speedometer({ 
  ...
  const handleSpeedChange = (delta: number) => {
    if (onSpeedChange) {
      const newSpeed = Math.max(0, Math.min(maxSpeed, speed + delta));
  ...
  return (
    <div>
      ...
      {/* Speed Arc */}
      <div className="absolute inset-1 sm:inset-2 rounded-full border-4 sm:border-6 lg:border-8 border-transparent border-t-cyan-400 border-r-cyan-400"></div>
      ...
      {/* Speed Labels */}
      <div className="absolute inset-0 text-xs text-cyan-400 font-mono">
        <div className="absolute text-xs sm:text-sm" style={{ left: '15%', top: '85%' }}>0</div>
        <div className="absolute text-xs sm:text-sm" style={{ left: '25%', top: '70%' }}>87</div>
        <div className="absolute text-xs sm:text-sm" style={{ left: '50%', top: '10%' }}>175</div>
        <div className="absolute text-xs sm:text-sm" style={{ left: '75%', top: '70%' }}>262</div>
        <div className="absolute text-xs sm:text-sm" style={{ left: '85%', top: '85%' }}>350</div>
      </div>
    ...
    {onSpeedChange && (
      <div className="flex justify-center space-x-1 sm:space-x-2 mt-2 sm:mt-4">
        <button
          ...
          title="Decrease Speed by 10 km/h"
        >
        ...
        <button
          ...
          className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-700 hover:bg-gray-600 text-white rounded-full flex items-center justify-center transition-colors"
          title="Decrease Speed by 1 km/h"
        >
          <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          onClick={() => handleSpeedChange(1)}
          className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-700 hover:bg-gray-600 text-white rounded-full flex items-center justify-center transition-colors"
          title="Increase Speed by 1 km/h"
        >
          <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          onClick={() => handleSpeedChange(10)}
          className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-700 hover:bg-gray-600 text-white rounded-full flex items-center justify-center transition-colors"
          title="Increase Speed by 10 km/h"
        >
          <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
